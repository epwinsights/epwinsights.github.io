...
export function renderRelativeHumidityCompareCharts(epwDataA, epwDataB) {
...
export function renderRHCompareControls(panelSelector) {
...
export function renderRHDistributionComparison(selector, dataA, dataB) {
  ...
  addInfoButton(selector, 'compareRHDistribution');
...
export function renderRHDiurnalComparison(selector, dataA, dataB) {
  const container = d3.select(selector).html('');
  const locNameA = formatSimpleLocation(dataA.metadata.location.city, dataA.metadata.location.country, 'primary');
  const locNameB = formatSimpleLocation(dataB.metadata.location.city, dataB.metadata.location.country, 'comparison');
  const legendNameA = formatCityNameOnly(dataA.metadata.location.city, 'primary');
  ...
  addInfoButton(selector, 'compareRHDiurnal');
